...
try:  
   ...
   print("Please set the environment variable MARA_PATH", end='')
   sys.exit(1)
...
os.chdir(os.path.join("./", maraPath))
args = ["./mara.sh","-s",sys.argv[1]]
try:
    subprocess.call(args, timeout=10 * 60, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
except subprocess.TimeoutExpired:
    print('Error')
    sys.exit(0)

if os.path.exists(os.path.join("./data", apk, "smali/apktool/")):
	apkSmaliPath = os.path.join("./data", apk, "smali/apktool/")
elif os.path.exists(os.path.join("./data", apk, "smali/baksmali/")):
	apkSmaliPath = os.path.join("./data", apk, "smali/baksmali/")
crt = open(apk+".txt","w")
		...
		with open(os.path.join(iroot,name),"r") as f:
			for x in f:
				if re.search('invoke-.*(Landroid).*',x):
					print((str(re.findall('\w+',x.split('}')[1].split('-')[0]))).translate(str.maketrans('', '','[],\'')).replace(" ",".")[1:], end =" ", file = crt)
					print(':', end = " ", file = crt)
					print(re.findall('\w+',x.split('}')[1].split('-')[1])[0],file = crt)
shutil.rmtree(os.path.join("./data", apk))
...
with open(apk+".txt", "r") as cur1:
	for x in cur1:
		if(len(x.split(":")) > 1):
			cname = str(re.findall("\w+",x.split(":")[0])).translate(str.maketrans('', '','[],\'')).replace(" ",".")
			mname = str(re.findall("\w+",x.split(":")[1])).translate(str.maketrans('', '','[],\''))
...
for x in lineset:
	...
	for j in range(len(API_ALLOWED_LIST)):
		if API_ALLOWED_LIST[j] == x.strip("\n"):
			...
			fv[j] = 1
print(str(fv).translate(str.maketrans('', '','[]')))  
print(''.join(list(lineset)))
